...
from mjlab.rl import CollectionEnv
...
class TrainConfig:
  ...
  collection_layers: list[str] = field(default_factory=lambda: ["mlp"])
  ...
  def from_task(task_id: str) -> "TrainConfig":
    ...
    collection_cfg = load_collection_cfg(task_id)
    return TrainConfig(
      env=env_cfg,
      agent=agent_cfg,
      collection_layers=collection_cfg.layers if collection_cfg else ["mlp"],
    )
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  if cfg.collection_db:
    db_uri = (
      cfg.collection_db
      if "://" in cfg.collection_db
      else f"sqlite:///{cfg.collection_db}"
    )
  else:
    db_name = "interlatent.db" if rank == 0 else f"interlatent_rank{rank}.db"
    db_uri = f"sqlite:///{log_dir / db_name}"
  db = LatentDB(db_uri)
  env = CollectionEnv(
    env,
    db=db,
    hook_layers=cfg.collection_layers,
  ...
  if cfg.collect and isinstance(env, CollectionEnv):
    tags = {
      "task": task_id,
    ...
    if cfg.agent.run_name:
      tags["run_name"] = cfg.agent.run_name
    with env.collecting(
      runner.alg.actor,
      run_id=cfg.collection_run_id,
      tags=tags,
    ):
      runner.learn(
