...
from mjlab.rl.collection_env import CollectionEnv
...
class MjlabCollectingOnPolicyRunner(OnPolicyRunner):
  """Base runner that persists environment state across checkpoints."""
  ...
  def __init__(
    ...
    for key in ("actor", "critic"):
      if key in train_cfg and train_cfg[key].get("cnn_cfg") is None:
        train_cfg[key].pop("cnn_cfg", None)
    super().__init__(env, train_cfg, log_dir, device)
    if isinstance(self.env, CollectionEnv):
      self.env.attach(self.alg.actor)
    else:
      raise ValueError("MjlabCollectingOnPolicyRunner requires a CollectionEnv.")
